...
```{r knitr_setup, message=FALSE, warning=FALSE, error= FALSE}
...
https://raw.githubusercontent.com/SimonCoulombe/saaqmtq/master/blog_saaq.Rmd 


## Define functions, downloads files
```{r functions_download}
...
save(geocode_this, geo, file= paste0("geo_",city,"_",type,".rdata"))
} else {load(paste0("geo_",city,"_",type,".rdata"))}
...
datatable(data =  accidents %>% filter(gravite== "Mortel") %>% select(DT_ACCDN, gravite, location, NAME_MUNCP, NO_CIVIQ_ACCDN, RUE_ACCDN, ACCDN_PRES_DE, TP_REPRR_ACCDN, ACCDN_PRES_DEmod, ACCDN_PRES_DE)  %>%
          ...
          caption = paste0("List of the ", accidents   %>% filter(gravite == "Mortel") %>% tally(), " deadly crashes involving ", type," in ", citytext, ", 2011 to 2016"))
```
...
## Top 10 locations with the most crashes   {.tabset}

The table below shoes the top 10 of the locations with the most crashes involving 
`r type` in  `r citytext ` between 2011 and 2016.

If we used the data from the whole province then we generate a top 10 for each 
cities below.
```{r add_accident}
...
worst10 <- geocode_this %>% group_by(lat,lon) %>% slice(1:1) %>% ungroup() %>% select(accidents, location, lon, lat)  %>% arrange(desc(accidents))   %>% slice(1:10)


datatable(data = worst10  %>% select(-lat, -lon),
          rownames = F,
          options = list(dom = 't', ordering = F, pageLength=10) ,
          caption = paste0("Top  10 locations with the most geocoded crashes involving ", type, " in ", citytext," between 2011 and 2016)"))
...
## List of car crashs at the top 10 most dangerous spots

The table below lists all the crashes that occurred at the top 10 most dangerous
spots.  
```{r liste_most_dangerous_spot}
datatable(data = worst10 %>%
            inner_join(geocode_this %>% select(DT_ACCDN, gravite,lon, lat, NO_CIVIQ_ACCDN ,
RUE_ACCDN,
ACCDN_PRES_DE)) %>% select(-lat, -lon) ,
          rownames = F,
          options = list(dom = 'pt', ordering = F, pageLength=10) ,          
          caption = paste0("List of the crashes that occurred at the top 10 spots of  ", citytext, " between 2011 and 2016"))
```
...
It would also be very interesting to know at which spots the rato of crashes per travel is high. 
